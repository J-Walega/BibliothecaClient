<template>
  <v-app-bar color="primary">
    ...
    <v-btn class="bg-red-lighten-5 me-6" @click="goToHome()"> Strona główna </v-btn>
    <v-btn prepend-icon="mdi-magnify" class="bg-red-lighten-5 me-6"
      ...
      <v-dialog v-model="searchDialog" activator="parent" width="500px">
        <v-card>
          ...
          <v-card-actions>
            <v-btn color="primary" block @click="searchQuery(searchText)">Wyszukaj</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </v-btn>
    <div class="d-flex justify-space-around">
      <v-menu>
        <template v-slot:activator="{ props }">
          <v-btn class="me-6" color="gray" icon="mdi-account-circle" v-bind="props"></v-btn>
        </template>
        <v-list>
          ...
          <v-list-item v-if="authStore.user?.user.roles.includes('Administrator')">
  ...
  <v-snackbar :timeout="2000" v-model="snackbar" color="success"> Wylogowano pomyślnie </v-snackbar>
</template>
